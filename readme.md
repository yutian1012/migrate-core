...
查询系统操作抛出异常的记录（未找到原因）
select * from log_model where data_id not in (select o_id from patent_info_check) and status='fail';


=============================================================================
批次的执行：查看BatchController类的migreate方法
1）执行批次前先记录批次执行日志 batchLogModel，状态为BatchStatusEnum.PROCESSING(处理中)

2）获取批次中的表，分别执行表的迁移操作
注：由于表迁移操作是通过线程池执行的，因此是异步的操作，

3）线程提交到线程池中，方法就返回了，更新日志的执行状态为已完成

存在的问题：
1）线程池大小固定，如果执行的批次数据很多，会操作等待线程池，方法不能立即返回--可使用消息队列的形式进一步解决异步问题

2）线程池中的操作并没有结束，而批次执行的状态已经变成了已完成。这种方式非常不准确。
可通过计算出批次待执行的数据量进一步来设置状态，数据执行前计算出迁移的结果集大小，真正执行完成后，能统计出执行的数量，通过两者比较完成状态的更新操作。
