...
数据迁移工具的应用场景:专利数据从trs库迁移到关系型数据库（可结合mongodb进行存储），实现对专利数据进行进一步的挖掘。

执行迁移操作的控制：前后执行关系，暂停，恢复，进度查看等。

修改完成后的处理，分为2种方式（方式1制定新的处理类，如格式化操作类，方式2手工处理然后更新处理数据），在日志的基础上添加处理方式，需要对错误分类做的更细致，针对没一类都可以进行手工处理（如申请号中有一位误操作，只需手动更正该位即可）和再次

批次执行的操作（对这类错误进行了错误逻辑处理，然后再次执行更正完成）。

数据验收标准，指定什么样的数据是合法的，什么样的数据是不合法的。方便更精确的处理数据。可针对没一个记录字段进行单独处理，对字段设定标准连，标准连从前向后执行，一旦不符合该标准的规则，则抛出异常。异常将交由相应的标准处理函数连来完成，完成

后继续执行标准验收连，都处理完成后返回最终的数据集。然后将数据集保存到数据库中，完成数据的清洗操作。
